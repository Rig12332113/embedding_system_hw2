...
#endif // MBED_CONF_APP_USE_TLS_SOCKET
*/
 static constexpr size_t REMOTE_PORT = 12001;
...
public:
    ...
    void run()
    {
        ...
        while (1) {
            ...
            temper_value = BSP_TSENSOR_ReadTemp();
            // printf("\nTEMPERATURE = %.2f degC\n", sensor_value);
            ...
            float x = pAcceleroXYZ[0]*SCALE_MULTIPLIER, y = pAcceleroXYZ[1]*SCALE_MULTIPLIER, z = pAcceleroXYZ[2]*SCALE_MULTIPLIER;
            ...
            /*int len = sprintf(json_buf, "%f %f %f %d %d %d %.2f %.2f %.2f %d %d %d\n", \
                                temper_value, humidity_value, pressure_value, \
                                pMagnetoXYZ[0], pMagnetoXYZ[1], pMagnetoXYZ[2], \
                                pGyroXYZ[0], pGyroXYZ[1], pGyroXYZ[2],\
                                pAcceleroXYZ[0], pAcceleroXYZ[1], pAcceleroXYZ[2]);
            response = _socket.send(json_buf,len);
            if (0 >= response){
                printf("Error seding: %d\n", response);
            }*/
            int len = sprintf(json_buf,"{\"x\":%f,\"y\":%f,\"z\":%f,\"s\":%d}",(float)((int)(x*10000))/10000, (float)((int)(y*10000))/10000, (float)((int)(z*10000))/10000, sample_num);
                response = _socket.send(json_buf,len); 
            if (0 >= response){
                printf("Error seding: %d\n", response); }
